...
def showScore(x, y):
    scoreValue = scoreFont.render("Score: " + str(score), True, green)
...
for i in range(numOfEnemies):
    ...
    enemyYPosition.append(0)  # Giving enemy a position on Y-axis starting from 0 i.e starting
    enemyXPositionChange.append(
        0)  # Initial speed of enemy in X direction is, 0 because its 0, then it will not move because it will not move horixontally
    enemyYPositionChange.append(
        0.4)  # Initial speed of enemy in y direction is, 0.7 downwards because if its 0, then it will not move because there is no key function given it to move
...
while running:
    ...
    for i in range(numOfEnemies):
        ...
        # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        enemyYPosition[i] += enemyYPositionChange[i]  # Increasing the position in Y direction for moving the enemy #Enemy moves only in Y direction..as no X position given
        # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if collisionOccured:  # If collOccured value returns True i.e collided
            ...
            score += 1  # if collided or hit the enemy then increase the score by 1
            print(score)
            enemyXPosition[i] = random.randint(0,
                                            735)  # Bring the enemy back to its default position and again a new enemy occurs from random position on horizontal starting line
            enemyYPosition[i] = 0  # And its Y position is 0 because it has to start from up
        # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    for i in range(numOfEnemies):
        if enemyYPosition[i] + 32 >= playerYPosition:
            for j in range(numOfEnemies):
                enemyYPosition[j] = 2000
            overFont = pygame.font.Font("freesansbold.ttf", 60)
            gameOver = overFont.render("GAME OVER!!!", True, red)
